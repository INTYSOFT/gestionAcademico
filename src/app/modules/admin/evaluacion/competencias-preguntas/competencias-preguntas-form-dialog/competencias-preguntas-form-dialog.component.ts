...
export class CompetenciasPreguntasFormDialogComponent {
    ...
    protected save(): void {
        ...
        const evaluacionTipoPregunta = this.data.evaluacionTipoPregunta ?? null;
        this.isSaving$.next(true);

        const request$ = evaluacionTipoPregunta
            ? this.evaluacionTipoPreguntasService.updateEvaluacionTipoPregunta(
                  evaluacionTipoPregunta.id,
                  payload
              )
        ...
        request$
            ...
            .subscribe({
                next: (result) => {
                    const action = evaluacionTipoPregunta ? 'updated' : 'created';
                    const message =
                        action === 'created'
                            ? 'Competencia de pregunta registrada correctamente.'
                            : 'Competencia de pregunta actualizada correctamente.';
                ...
                error: (error) => {
                    this.snackBar.open(
                        error.message ??
                            'Ocurrió un error al guardar la competencia de pregunta de evaluación.',
                        'Cerrar',
                        {
                            duration: 5000,
                        }
