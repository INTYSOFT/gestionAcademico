...
export class CompetenciasPreguntasComponent implements OnInit, OnDestroy {
    ...
    protected readonly columnDefs: ColDef<EvaluacionTipoPregunta>[] = [
        ...
        { headerName: 'Código', field: 'codigo', minWidth: 160 },
        ...
        },
        {
            ...
            cellRendererParams: {
                onEdit: (evaluacionTipoPregunta: EvaluacionTipoPregunta) =>
                    this.openFormDialog(evaluacionTipoPregunta),
            },
            width: 120,
            sortable: false,
            filter: false,
        ...
        this.evaluacionTipoPreguntasService
            ...
            .subscribe({
                ...
                error: (error) => {
                    this.snackBar.open(
                        error.message ??
                            'Ocurrió un error al cargar las competencias de preguntas de evaluación.',
                        'Cerrar',
                        {
                            duration: 5000,
                        }
    ...
    private applyFilter(term: string): void {
        ...
        const records = this.evaluacionTipoPreguntas$.value;

        if (!normalized) {
            this.filteredEvaluacionTipoPreguntas$.next([...records]);
            return;
        }
    ...
    private openFormDialog(evaluacionTipoPregunta?: EvaluacionTipoPregunta | null): void {
        blurActiveElement();

        void import(
            './competencias-preguntas-form-dialog/competencias-preguntas-form-dialog.component'
        ).then(({ CompetenciasPreguntasFormDialogComponent }) => {
